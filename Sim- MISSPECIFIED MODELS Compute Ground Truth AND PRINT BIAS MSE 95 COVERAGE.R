...
##################################################################################
########## Coefficients for a misspecified data generating model######
##################################################################################
...
param_reg_df_Y2_input_list = list(param_reg_df_Y2_true)
...
param_reg_df_Y3_input_list = list(param_reg_df_Y3_true)
...
param_reg_df_ZLM_input_list = list(param_reg_df_ZLM_true)   
...
get_parametric_coeffs_iter = function(param_reg_df_Y2,   #regression data for fitting Y2 model
  ...
  #formula_Y2= as.formula("Y_2 ~ V0 + BMI + I(BMI^2) + AGE+ I(AGE^2) + RACE + SEX + EDU + Z_1 + L_1 + 
  #                       M_1")
  
  if(is_binary(param_reg_df_Y2$Y_2) == TRUE){
    fit_glm_Y2 = glm(Y_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + 
                       M_1, data = param_reg_df_Y2, family = binomial(link="logit"))
    ...
    fit_glm_Y2 = lm(Y_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + 
                      M_1, data = param_reg_df_Y2)
    fit_glm_Y2_coeff = as.data.frame(coef(fit_glm_Y2))
    fit_glm_Y2_coeff = rbind(fit_glm_Y2_coeff[1:5, , drop = FALSE], 
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_Y2_coeff[6:nrow(fit_glm_Y2_coeff), , drop = FALSE],
                             sigma = sigma(fit_glm_Y2))
  }
  
  
  
  
  # formula_Y3= as.formula("Y_3 ~ V0 + BMI + I(BMI^2) + AGE+ I(AGE^2) + RACE + SEX + EDU + Z_1 + L_1 
  #                 + M_1 + Z_2 + L_2 +M_2")
  if(is_binary(param_reg_df_Y3$Y_3) == TRUE){
    fit_glm_Y3 = glm(Y_3 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + M_1
                     + Z_2 + L_2 +M_2, data = param_reg_df_Y3, family = binomial(link="logit"))
    ...
    fit_glm_Y3 = lm(Y_3 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + M_1
                    + Z_2 + L_2 +M_2, data = param_reg_df_Y3)
    fit_glm_Y3_coeff = as.data.frame(coef(fit_glm_Y3))
    fit_glm_Y3_coeff = rbind(fit_glm_Y3_coeff[1:5, , drop = FALSE],
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_Y3_coeff[6:nrow(fit_glm_Y3_coeff), , drop = FALSE],
                             sigma = sigma(fit_glm_Y3))
  }
  ...
  if(is_binary(param_reg_df_ZLM$Z_1) == TRUE){
    fit_glm_Z1 = glm(Z_1 ~ V0 +  RACE + SEX + EDU, 
                     data = param_reg_df_ZLM, family = binomial(link="logit"))
    fit_glm_Z1_coeff = as.data.frame(coef(fit_glm_Z1))
    fit_glm_Z1_coeff = rbind(fit_glm_Z1_coeff[1:5, , drop = FALSE], 
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_Z1_coeff[6:nrow(fit_glm_Z1_coeff), , drop = FALSE])
  }else{
    fit_glm_Z1 = lm(Z_1 ~ V0 +  RACE + SEX + EDU,
                    data = param_reg_df_ZLM)
    fit_glm_Z1_coeff = as.data.frame(coef(fit_glm_Z1))
    fit_glm_Z1_coeff = rbind(fit_glm_Z1_coeff[1:5, , drop = FALSE], 
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_Z1_coeff[6:nrow(fit_glm_Z1_coeff), , drop = FALSE],
                             sigma = sigma(fit_glm_Z1))
  }
  ...
  if(is_binary(param_reg_df_ZLM$Z_2) == TRUE){
    fit_glm_Z2 = glm(Z_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 +   
                       M_1, data = param_reg_df_ZLM, family = binomial(link="logit"))
    ...
    fit_glm_Z2 = lm(Z_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 +   
                      M_1, data = param_reg_df_ZLM)
    fit_glm_Z2_coeff = as.data.frame(coef(fit_glm_Z2))
    fit_glm_Z2_coeff = rbind(fit_glm_Z2_coeff[1:5, , drop = FALSE], 
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_Z2_coeff[6:nrow(fit_glm_Z2_coeff), , drop = FALSE],
                             sigma = sigma(fit_glm_Z2))
  }
  ...
  if(is_binary(param_reg_df_ZLM$L_2) == TRUE){
    fit_glm_L2 = glm(L_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + M_1
                     + Z_2, data = param_reg_df_ZLM, family = binomial(link="logit"))
    ...
    fit_glm_L2 = lm(L_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + M_1
                    + Z_2, data = param_reg_df_ZLM)
    fit_glm_L2_coeff = as.data.frame(coef(fit_glm_L2))
    fit_glm_L2_coeff = rbind(fit_glm_L2_coeff[1:5, , drop = FALSE], 
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_L2_coeff[6:nrow(fit_glm_L2_coeff), , drop = FALSE],
                             sigma = sigma(fit_glm_L2))
  }
  ...
  if(is_binary(param_reg_df_ZLM$M_2) == TRUE){
    fit_glm_M2 = glm(M_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + M_1
                     + Z_2 + L_2, data = param_reg_df_ZLM, family = binomial(link="logit"))
    ...
    fit_glm_M2 = lm(M_2 ~ V0 +  RACE + SEX + EDU + Z_1 + L_1 + M_1
                    + Z_2 + L_2, data = param_reg_df_ZLM)
    fit_glm_M2_coeff = as.data.frame(coef(fit_glm_M2))
    fit_glm_M2_coeff = rbind(fit_glm_M2_coeff[1:5, , drop = FALSE], 
                             BMI_lg_coeff, AGE_lg_coeff, BMIXSEX_coeff,
                             fit_glm_M2_coeff[6:nrow(fit_glm_M2_coeff), , drop = FALSE],sigma = sigma(fit_glm_M2))
  }
...
pred_comb_mediator_trt <- function(cont, param_model_coeff, MCdata_trt, MCdata_control){
  ...
  if(cont == FALSE) {
    ...
    new_MCdata = cbind(MCdata_trt,new_predicted_M,log(new_predicted_M))
    #new_MCdata = cbind(MCdata_trt,log(new_predicted_M))
  }
  #print(new_MCdata)
  return(new_MCdata)
}
...
pred_comb_mediator_ctrl <- function(cont,# TRUE if the given var is continuous
  ...
  if(cont == FALSE) {
    ...
    new_predicted_M = rnorm(length(x_hat), mean = x_hat, sd = sd_hat)
    new_MCdata = cbind(MCdata, new_predicted_M, log(new_predicted_M))
    #new_MCdata = cbind(MCdata, log(new_predicted_M))
  }
  return(new_MCdata)
}
...
input_data1 = data.frame(V0 = ARIC_workdf_wide$SMOKE_STATUS_MULT_1, 
                         RACE = ARIC_workdf_wide$RACE, 
                         SEX = ARIC_workdf_wide$SEX, 
                         EDU = ARIC_workdf_wide$EDU, 
                         BMI_lg = log(ARIC_workdf_wide$BMI_1),
...
input_varType1 = c("L0","L0","L0","L0","L0","L0","L0","Fi", "L", "M","Y", "Fi", "L", "M", "Y","D")
...
sim_BART_result_df = data.frame(data.table::fread("~/Documents/My Computer/Discrete time Mediation Analysis/Codes and Results/Project Coding/GcompBART2/R/Results_ARIC_BART_MissModels_result.txt", sep = "\t"))

head(sim_BART_result_df)


sim_Parametric_result_df = data.frame(data.table::fread("~/Documents/My Computer/Discrete time Mediation Analysis/Codes and Results/Project Coding/GcompBART2/R/Results_ARIC_Stan_Param_MissModels_result.txt", sep = "\t"))
