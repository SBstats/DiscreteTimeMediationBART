...
txt.title = paste0("param_simu_results/Results_ARIC_BART_Models_result.txt")
if (run_ID == 1) {
  ...
  for (i in (n_L0+1):(ncol(data)-1)) {  #(numL0Vars+1)
    ...
    if(var.type[1+i] == "Fi") {
      ...
      if (continuous[1+i] == FALSE & Suppress == TRUE) {
        ...
        #sum_sq_resid <- sum((data[id == 1, (1 + i)] - BModels[[i]]$prob.train.mean)^2)
        #plot(sum_sq_resid, type = "l", xlab = "Iteration", ylab = "Sum of Squared Residuals")
        #acf(sum_sq_resid,type = "correlation",plot= TRUE)
        ...
        #sigma_sq <- (BModels[[i]]$sigma)^2
        #plot(sigma_sq, type = "l", xlab = "Iteration", ylab = "Sigma Squared")
        #acf(sigma_sq,type = "correlation",plot= TRUE)
...
P_zzstar = MCInteg(data = input_data1,
                   ...
                   #endBaselineAge = 48,
                   J=10000,          #10000
                   Ndraws= 1000,                  #set to 1 when we are computing the ground truth
                   #Nskip = 1000, 
                   #Ntree = 100, 
                   #Keepevery = 5, 
                   Suppress = TRUE, 
